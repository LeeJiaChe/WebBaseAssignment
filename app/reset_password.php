...
// Security Check: User must have passed OTP verification
if (empty($_SESSION['can_reset_password']) || empty($_SESSION['reset_email'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $db->prepare("UPDATE users SET password_hash = ?, reset_otp = NULL, reset_otp_expiry = NULL WHERE email = ?");
        if ($stmt->execute([$hash, $email])) {
            ...
            $errors[] = 'Database error. Failed to update password.';
        }
    }
}
        ...
        <div class="auth-content">
            ...
            <form method="post" action="" novalidate>
                <div class="form-group">
                    ...
                    <input id="password" name="password" type="password" required class="form-input" minlength="6" autofocus>
                </div>
                <div class="form-group">
